...
```{r}
#Pull data (example is for December 2021)
...
df2 <- data.frame(df1$data.src_actor, df1$data.tgt_actor)

#Extract USA, DEU, GBR, RUS, UKR, CHN, IND, and TUR as actor pairs from "src_actor" and "tgt_actor" categories 
df3 <- filter(df2, df1.data.src_actor %in% c('USA', 'DEU', 'GBR', 'RUS', 'UKR', 'CHN', 'IND', 'TUR'))

df4 <- filter(df3, df1.data.tgt_actor %in% c('USA', 'DEU', 'GBR', 'RUS', 'UKR', 'CHN', 'IND', 'TUR'))

#Create dataframe sorted alphabetically over source actor (to take control back from igraph's node positioning)
df5 <- df4[order(df4$df1.data.src_actor), , drop = FALSE]
```

```{r}
#Create graph object

g <- graph_from_data_frame(df5)
```
